...
const LoginAdmin = () => {
  ...
  const iniciarSesion = async () => {
    try {
      const response = await APIInvoke.invokeGET(`/Usuarios?email=${admin.email}`);

      if (response.length === 0) {
        const msg = "El administrador no existe. Verifique los datos ingresados.";
        swal({
          title: 'Error',
          text: msg,
          icon: 'error',
    ...
    <div className="hold-transition login-page">
      <div className="login-box">
        <div className="login-logo">
          <Link to={"#"}>
            <b>Iniciar </b> Sesión Administrador
          </Link>
        </div>
